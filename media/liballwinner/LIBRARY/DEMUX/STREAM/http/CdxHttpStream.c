    ...
    while(impl->validDataSize < len)
    {
        ...
        else if(impl->ioState == CDX_IO_STATE_ERROR)
        {
            ...
            while(1)
            {
                ...
                if(ret == 0)
                {
                    CDX_LOGD("reconnect ok, continue read.");
                    pthread_mutex_unlock(&impl->bufferMutex);
                    break;
                }
...
static cdx_int32 StreamRead(CdxHttpStreamImplT *impl, void *buf, cdx_uint32 len, cdx_bool *isEos)
{
    ...
    *isEos = 0;
    ...
    {
        //----------------------------------------for test mgtv fast-----------
        //if(impl->bufPos > 20*1024*1024)
        //{
        //    CDX_LOGE("xxx test mgtv impl->bufPos(%lld)> 20M, socket closed by peer?", impl->bufPos);
        //    //impl->ioState = CDX_IO_STATE_ERROR;
        //    impl->ioState = CDX_IO_STATE_EOS;//ok
        //    goto err_out; 
        //}
        ...
        {
            CDX_LOGD("xxx readLen=0, socket closed by peer?");
            //if(impl->totalSize != -1)
            //    impl->ioState = CDX_IO_STATE_ERROR;
            //else
            //    impl->ioState = CDX_IO_STATE_EOS;
...
void *GetNetworkData(void *pArg)
{
    ...
    while(1)
    {
        ...
        if(impl->ioState == CDX_IO_STATE_OK) //after seek may io error or eos.
        {
			...
			{
				impl->downloadEnd = GetNowUs();
				impl->downloadTimeUs = (impl->downloadEnd - impl->downloadStart) /1000;
				if(impl->callback && callbackEndFlag == 0)
				{
					 CDX_LOGD("eos set STREAM_EVT_DOWNLOAD_END ");
					impl->callback(impl->pUserData, STREAM_EVT_DOWNLOAD_END, &(impl->downloadTimeUs));
...
#if 0
    if((CdxHttpHeaderFieldsT *)source->extraData)//set extra http header.
    {
        impl->nHttpHeaderSize = ((CdxHttpHeaderFieldsT *)source->extraData)->num;      
        impl->pHttpHeader = (CdxHttpHeaderFieldT *)Palloc(impl->nHttpHeaderSize * sizeof(CdxHttpHeaderFieldT));
        if(impl->pHttpHeader == NULL)
        {
            CDX_LOGE("Palloc failed.");
            goto err_out;
        }
        memcpy(impl->pHttpHeader, ((CdxHttpHeaderFieldsT *)source->extraData)->pHttpHeader, impl->nHttpHeaderSize * sizeof(CdxHttpHeaderFieldT));
    }
