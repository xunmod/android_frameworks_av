...
{
#ifdef ENABLE_INFO_DEBUG
    ATRACParserImpl *atrac;
    atrac = (ATRACParserImpl *)Parameter;

    ALOGV("SampleRate %d", atrac->mSampleRate);
    ALOGV("Channels %d", atrac->mChannels);
    ALOGV("BitRate %d", atrac->mBitrate);
    ALOGV("mFrameSize %d", atrac->mFrameSize);
    ALOGV("mDuration %lld", atrac->mDuration);
    ALOGV("mFileSize %lld", atrac->mFileSize);
#endif
    return 0;
}
