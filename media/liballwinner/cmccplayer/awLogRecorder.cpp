...
static void CreateFile(AwLogRecorder l)
{
    ...
    static char playLogPath[] = "/tmp/playLog/";
...
int AwLogRecorderRecord(AwLogRecorder* l, char* url, unsigned int nPlayTimeMs, int status)
{
    ...
    lc = (AwLogRecorderContext*)l;
    if(nPlayTimeMs > 259200)
    {
    	nPlayTimeMs = 0;
    }
    //* send a set data source message.
    logd("++++++++++ lc =%p, new message, status = %d, nPlayTimeMs = %d", lc, status, nPlayTimeMs);
    setMessage(&msg, 
               ...
               (unsigned int)url,               //* params[2] = url.
               (unsigned int)nPlayTimeMs,       //* params[3] = nPlayTimeMs.
               (unsigned int)status);           //* params[4] = status;
    logd("+++++++++>>lc->mq = %p, param2 = %d, param3 = %d, param4 = %d.", lc->mq,msg.params[2], msg.params[3], msg.params[4]);
    AwMessageQueuePostMessage(lc->mq, &msg);
    SemTimedWait(&lc->semQuit, -1);
    ...
    while(1)
    {
        if(AwMessageQueueTryGetMessage(lc->mq, &msg, 5000) == 0)
        {
            ...
            if(msg.messageId == LOGRECORDER_MSG_NEWLOG)
            {
                ...
                url         = (char*)msg.params[2];
                nPlayTimeMs = msg.params[3];
                status      = msg.params[4];
                //ALOGD("+++++++++<< param2 = %d, param3 = %d, param4 = %d.", msg.params[2], msg.params[3], msg.params[4]);
                //AddOneMessage(lc, url, nPlayTimeMs, status);
                ...
                {
                    char buf[4096];
                    TimeFormatTransform(nSysTime, buf, "#systemtime:", "");
                    ret = fprintf(lc->fp, "%s\n", buf);
                    if(ret < 0)
                    {
					...
					if(url == NULL)
					{
						sprintf(buf, "%s%s", "url:", "test");
					}
					else
					{
                    	sprintf(buf, "%s%s", "url:", url);
                    }
                    fprintf(lc->fp, "%s\n", buf);
                    sprintf(buf, "%s%d", "playtime:", nPlayTimeMs);
                    fprintf(lc->fp, "%s\n", buf);
                    if(status == MEDIA_INFO_DOWNLOAD_ERROR)
                        sprintf(buf, "%s%d", "status:", 703);   //* according to the China Mobile's spec 2.1.7.
                    else
                        sprintf(buf, "%s%d", "status:", status);
                    fprintf(lc->fp, "%s\n", buf);
                    if(status == MEDIA_INFO_BUFFERING_START)
                    {
                    	lc->bufferStartFlag = 1;
                    	sprintf(lc->url, "%s", url);
                    	lc->nPlayTime = nPlayTimeMs;
                        ALOGD("log: bufferStart");
                        sprintf(buf, "%s%s", "desc:", "卡顿开始");
                        fprintf(lc->fp, "%s\n", buf);
                    }
                    else if(status == MEDIA_INFO_BUFFERING_END)
                    {
                    	lc->bufferStartFlag = 0;
                        ALOGD("log: bufferEnd");
                        sprintf(buf, "%s%s", "desc:", "卡顿结束");
                        fprintf(lc->fp, "%s\n", buf);
                    }
                    else if(status == MEDIA_INFO_DOWNLOAD_ERROR)
                    {
                        sprintf(buf, "%s%s%s", "desc:", "获取不到切片，切片url是", url);
                        fprintf(lc->fp, "%s\n", buf);
                    }
                    else
                    {
                        sprintf(buf, "%s%s", "desc:", "未知的播放器错误");
                        fprintf(lc->fp, "%s\n", buf);
                    }
                    fprintf(lc->fp, "\n");
                }
...
static void  SaveNewFile(AwLogRecorderContext* lc)
{
    ...
    //* set file name.
    TimeFormatTransform(lc->nFileStartTime, strFilePath, "/tmp/playLog/", ".txt");
