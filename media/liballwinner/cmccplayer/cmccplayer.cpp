...
#if PAUSE_THEN_SEEK_BUG
	if( mStatus == AWPLAYER_STATUS_PAUSED || mStatus == AWPLAYER_STATUS_STARTED)
	{
		int pos;
...
status_t CmccPlayer::getCurrentPosition(int* msec)
{
       ...
       mStatus == AWPLAYER_STATUS_COMPLETE)
    {
        ...
        if(mLivemode == 1)
        {
        	...
        	if(*msec < 0) *msec = 0;
        	logd("livemode = 1, getCurrentPosition %d", *msec);
            return OK;
        }
        else if(mLivemode == 2)
        {
    ...
    switch(messageId)
    {
        ...
        case AWPLAYER_MESSAGE_DEMUX_BUFFER_START:
        {
			...
			if(mLogRecorder != NULL)
			{
				...
				{
		    		getCurrentPosition(&nPlayTime);
		    		logd("--- nPlayTime = %d", nPlayTime);
			    }
				logd("buffer start url = %s", (unsigned char*)param);
				memcpy(mUri, (unsigned char*)param, 1024);
			    AwLogRecorderRecord(mLogRecorder, mUri, (unsigned int)(nPlayTime), MEDIA_INFO_BUFFERING_START);
			}
        ...
        case AWPLAYER_MESSAGE_DEMUX_BUFFER_END:
        {
	        ...
	        if(mLogRecorder != NULL)
	        {
			    ...
			    //char uri[10] = "test";
	            AwLogRecorderRecord(mLogRecorder, mUri, (unsigned int)(nPlayTime), MEDIA_INFO_BUFFERING_END);
	        }
		...
		{
			logd("--- hls message, AWPLAYER_MESSAGE_DEMUX_HLS_DOWNLOAD_START");			
            sendEvent(MEDIA_INFO, MEDIA_INFO_DOWNLOAD_END, (*(int*)param));
            break;
		}
		case AWPLAYER_MESSAGE_DEMUX_HLS_DOWNLOAD_ERROR:
		{
			...
			if(mLogRecorder != NULL)
	        {
	            ...
	            AwLogRecorderRecord(mLogRecorder, obj->uri, (unsigned int)(nPlayTime), MEDIA_INFO_DOWNLOAD_ERROR);
	        }
